...
namespace FusionChart
{
    ...
    {
        public static string RenderChartByDataXML(DataTable data, string OrderType, string ChartType, string caption, string xAxisName, string yAxisName)
        {
            //过滤，排序
            ...
            {
                orderby = data.Columns[1].ColumnName + " " + OrderType;
            }
            DataView dv = new DataView(data, filter, orderby, DataViewRowState.CurrentRows);
            StringBuilder xmlData = new StringBuilder();
            xmlData.AppendFormat("<chart caption='{0}' xAxisName='{1}' yAxisName='{2}' showValues='1' formatNumberScale='0' showBorder='1' yAxisMaxValue='100' baseFontSize='12'>"
                , caption, xAxisName, yAxisName);
            for (int i = 0; i < dv.Count; i++)
            {
                xmlData.AppendFormat("<set label='{0}' value='{1}' />", dv[i][0].ToString(), dv[i][1].ToString());
            }
            xmlData.Append("</chart>");
            int width = dv.Count * 50;
            width = Math.Max(750, width);
            string dataXML = string.Empty;
            switch (ChartType)
            {
                case "0":
                    dataXML = FusionChart.FusionChartHelper.RenderChartByDataXML(FusionChart.FusionChartType.Column3D, xmlData.ToString(), width, 350);
                    break;
                case "1":
                    dataXML = FusionChart.FusionChartHelper.RenderChartByDataXML(FusionChart.FusionChartType.Pie3D, xmlData.ToString(), width, 350);
                    break;
                case "2":
                    dataXML = FusionChart.FusionChartHelper.RenderChartByDataXML(FusionChart.FusionChartType.Bar2D, xmlData.ToString(), width, 350);
                    break;
                case "3":
                    dataXML = FusionChart.FusionChartHelper.RenderChartByDataXML(FusionChart.FusionChartType.Line, xmlData.ToString(), width, 350);
                    break;
            }
            return dataXML;
        }
    ...
    public class BaseInfo
    {
        public static string FusionChartPath
        {
            get
            {
                if (System.Configuration.ConfigurationManager.AppSettings["FusionChartPath"] != null)
                {
                    return System.Configuration.ConfigurationManager.AppSettings["FusionChartPath"];
                }
                return "/FusionCharts";
            }
        }
    ...
    public enum FusionChartType
    {
        ...
        AngularGauge = 12, Cylinder = 13, Funnel = 14, HLinearGauge = 15,
        MSColumn3DLineDY = 16, ScrollColumn2D = 17
    }
    ...
    public class FusionChartHelper
    {
        ...
        public static Dictionary<int, string> ChartColors = new Dictionary<int, string>();
        public static string GetChartColor(int index)
        {
            //if (ChartColors.IsEmpty)
            //{
            //    DataTable dt = MainClass.GetDataTable("select ArgbValue from xn_chartcolor order by id");
            //    for (int i = 0; i < dt.Rows.Count; i++)
            //    {
            //        ChartColors.Add(i, dt.Rows[i][0].ToString());
            //    }
            //}
            if (ChartColors.ContainsKey(index) == false)
            {
                ChartColors.Add(index, new Random().Next(0, 255 * 255 * 255).ToString("X4"));
            }
        ...
        private static string RenderChart(FusionChartType chartType, string dataUrl, string dataXML, double chartWidth, double chartHeight, bool debugMode)
        {
            ...
            //setTransparent
            builder.AppendFormat("chart_{0}.setTransparent(true);" + Environment.NewLine, id);
